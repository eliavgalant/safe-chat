...
const Testimonials = () => {
  ...
  return <section id="testimonials" ref={sectionRef} className="container-padding relative overflow-hidden">
      {/* Background elements */}
      ...
      <div className="container mx-auto relative z-10">
        <div className="text-center max-w-3xl mx-auto mb-16">
          <h2 className="animate-on-scroll heading-lg text-black mb-4 shadow-text">המלצות</h2>
          <p className="animate-on-scroll rtl-text text-xl text-gray-600 shadow-text-light text-center">מה ההורים אומרים על השירות שלנו?</p>
        </div>

        <div className="max-w-6xl mx-auto">
          <div ref={sliderRef} className="grid grid-cols-1 md:grid-cols-3 gap-6 animate-fade-in">
            {testimonials.map((testimonial, index) => <div key={index} className="animate-on-scroll glass-card p-6 rounded-2xl relative h-full flex flex-col">
          ...
          <div className="flex justify-center mt-8 gap-4">
            <button onClick={() => handleScroll('prev')} className="p-2 rounded-full bg-white shadow-md hover:bg-safechat-gold hover:text-white transition-colors" aria-label="Previous testimonials">
              <ChevronRight className="w-5 h-5" />
            </button>
            
            <button onClick={() => handleScroll('next')} className="p-2 rounded-full bg-white shadow-md hover:bg-safechat-gold hover:text-white transition-colors" aria-label="Next testimonials">
